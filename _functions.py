...
def askForAvatar(empty_repr, rock_repr):
    avatar = None
    valid_avatar = False
    ...
    while (valid_avatar == False):
        ...
        entered_string = input(f"please input a single character that is not '{empty_repr}' or '{rock_repr}': ");

        for letter in entered_string:
            if ( (letter != empty_repr) and (letter != rock_repr) ): #If statement is to avoid making the player look like an empty space or a rock
...
def generateObjects(object_repr, level_arr, object_class):
    ...
    for y in range(len(level_arr)):
        ...
        for x in range(len(level_arr[y])):
            ...
            if (level_arr[y][x] == object_repr):
                new_rockObj = object_class(id_num = assigned_id, x=x, y=y, sprite = object_repr)
                objects_arr.append(new_rockObj);
...
#takes a objects_list and the map_arr, and will first clear the entire board and then add in the objects where they are needed
#edits the array, doesn't return a new copy
def objectsToMap(objects_list, map_arr, empty_repr, obj_repr):
    num_rows = len(map_arr)
